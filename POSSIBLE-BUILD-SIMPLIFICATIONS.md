...
**Ces chiffres ont été revérifiés le 31 juillet 2026 sur le DMG publié**, monté et relevé au `du` : 53 fichiers et 17 Mo de `qtwebengine_locales`, 1,5 Mo de `qtwebengine_devtools_resources.pak`, bundle de 208 Mo. Rien n'a bougé. Deux petites pièces s'ajoutent à ce que le script devrait retirer, du même genre et trouvées au même relevé : les greffons `position` (208 Ko) et le `QtSerialPort` (112 Ko) qu'ils sont seuls à faire entrer dans le bundle — rien ici n'utilise la géolocalisation, `macdeployqt` les copie parce que QtWebEngine déclare le module. C'est 0,3 Mo sur 18, donc à faire par cohérence et non pour le gain.
